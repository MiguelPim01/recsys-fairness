...
class GroupFairnessAnalyzer:
    ...
    def __init__(self, dataset_dir: str | Path, algorithm: str, config):
        self.dataset_dir = Path(dataset_dir)
        self.algorithm = algorithm
        self.config = config
        self.settings = dict(config["fairness"])

    def analyze(self, trainer, test_data, development_data: Iterable, recbole_metrics: dict[str, Any]):
        """
        Analyze recommendations fairness metric.

        Args:
            trainer: The training object.
            test_data: The test data.
            development_data (Iterable): The development data.
            recbole_metrics (dict[str, Any]): The RecBole metrics.
        ...
        for evaluation in evaluations:
            try:
                ...
                raise ValueError(f"Missing profile for test user {evaluation.user_id}") from error
    ...
    def _evaluate_users(self, trainer, test_data, topk):
        """
        Evaluates the performance of the model on a set of test users.

        Args:
            trainer: The training object.
            test_data: The test data.
            topk (list[int]): Topk values.

        Returns:
            evaluations: A list of dictionaries, each containing the evaluation metrics for a single user.
        """
                ...
                for row, internal_uid in enumerate(batch_users):
                    ...
                    for internal_iid in row_items:
                        ...
                        if not math.isfinite(predicted):
                            raise ValueError("Model produced a non-finite prediction for a test pair")
    ...
    def _development_activity(self, data_loaders: Iterable) -> Counter[str]:
        """
        Counts how many interactions each user has in the development data.

        Args:
            data_loaders (Iterable): Development data loaders to analyze.

        Returns:
            counts: A counter with the number of interactions for each user.
        """
    ...
    def _read_profiles(self):
        """
        Reads profiles from the dataset's user profile file.
    ...
    def _read_lastfm_profiles(reader, profile_path):
        """
        Reads LastFM user profiles.

        Args:
            reader: The CSV reader.
            profile_path: The path to the profile file.

        Returns:
            profiles: A dictionary mapping user IDs to their profiles. 
        """
        profiles = {}
    ...
    @staticmethod
    def _partition_metrics(partition: Partition, evaluations: list[UserEvaluation], topk: list[int]):
        """
        Calculates metrics for a given partition.

        Args:
            partition (Partition): The partition for which to calculate metrics.
            evaluations (list[UserEvaluation]): The list of user evaluations.
            topk (list[int]): The list of k values for NDCG calculation.
    ...
    @staticmethod
    def _validate_recbole_ndcg(detailed_ndcg: dict[str, float], recbole_metrics: dict[str, Any]):
        """
        Validates that the NDCG values calculated by the detailed method match those from RecBole.

        Args:
            detailed_ndcg (dict[str, float]): _description_
            recbole_metrics (dict[str, Any]): _description_
        """
