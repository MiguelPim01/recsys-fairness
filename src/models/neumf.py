...
class NeuMF(GeneralRecommender):
    ...
    Changes:
        - Added support for float numbers in the label and input field.
        - Changed the loss function to MSELoss for regression tasks.
    """
    ...
    def load_pretrain(self):
        ...
        for layer in self.mlp_layers.mlp_layers:
            if isinstance(layer, nn.Linear):
                ...
                ), "mlp layer parameter shape mismatch"
                assert (
        ...
        predict_weight = torch.cat([
            mf["predict_layer.weight"], 
            mlp["predict_layer.weight"]
        ], dim=1)
